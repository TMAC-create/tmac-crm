...
const [showCreditorAdmin, setShowCreditorAdmin] = useState(false);
...
const [clientSmsMessages, setClientSmsMessages] = useState<SmsMessage[]>([]);
...
function maxLoanAtLtv(targetLtv: number) {
  ...
  async function login() {
    ...
    await loadUnreadSmsCount(data.token);
    setView('dashboard');
  }
...
setCallbackForm({
  date: client.metadataJson?.callback?.date || '',
  time: client.metadataJson?.callback?.time || '',
  notes: client.metadataJson?.callback?.notes || '',
});
setCreditorSearch('');
  }
  ...
  async function openClient(client: Client) {
    ...
    await loadClientSmsMessages(client.id);
  }
  ...
  function mergeSmsTemplateText(body: string, client: Client | null) {
    if (!client) return body;
    return body
      .replaceAll('{{firstName}}', client.firstName || '')
      .replaceAll('{{lastName}}', client.lastName || '')
      .replaceAll('{{reference}}', client.reference ? String(client.reference) : '')
      .replaceAll('{{mobile}}', client.mobile || '')
      .replaceAll('{{email}}', client.email || '');
  }

  async function loadClientSmsMessages(clientId: string, activeToken = token) {
    ...
    if (!response.ok) {
      setError('Could not load SMS messages.');
      return;
    }

    const data = await response.json();
    setClientSmsMessages(data);
  }

  async function markClientSmsRead(clientId: string) {
    if (!token) return;
    ...
    await loadClientSmsMessages(clientId);
  }

  async function loadUnreadSmsCount(activeToken = token) {
    if (!activeToken) return;
    ...
    const data = await response.json();
    setUnreadSmsCount(data.count || 0);
  }

  function chooseSmsTemplate(templateId: string) {
    setSmsTemplateId(templateId);
    const template = templates.find((item) => item.id === templateId);
    if (template) {
      setSmsBody(mergeSmsTemplateText(template.body, selectedClient));
    }
  }

  async function sendSmsFromThread() {
    if (!selectedClientId || !smsBody.trim()) return;
    ...
    setSmsBody('');
    setSmsTemplateId('');
    await loadClientSmsMessages(selectedClientId);
    await loadUnreadSmsCount();
...
function resetTemplateForm(type: TemplateType = templateFilter) {
  setTemplateForm({ ...emptyTemplateForm, type });
...
function renderSmsTab() {
  const activeSmsTemplates = templates.filter((template) => template.type === 'SMS' && template.active);
  const sortedMessages = [...clientSmsMessages].sort(
    (a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
  );

  return (
    <section className="card tab-panel premium-panel sms-workspace">
      <div className="sms-workspace-header">
        <div>
          <h3>SMS Messages</h3>
          <p>Client SMS conversation, replies and outbound messages.</p>
        </div>
        {clientSmsMessages.some((message) => message.direction === 'INBOUND' && !message.readAt) && (
          <span className="sms-unread-pill">Unread replies</span>
        )}
      </div>
      ...
      <div className="sms-thread">
        {sortedMessages.length === 0 ? (
          <div className="sms-empty-state">
            ...
            <span>Choose a template below or type a manual message to start the thread.</span>
          </div>
        ) : (
          sortedMessages.map((message) => (
            <div key={message.id} className={'sms-bubble-row ' + (message.direction === 'OUTBOUND' ? 'outbound' : 'inbound')}>
              <div className={'sms-bubble ' + (message.direction === 'OUTBOUND' ? 'outbound' : 'inbound')}>
                <div className="sms-bubble-meta">
                  <strong>{message.direction === 'OUTBOUND' ? 'TMAC' : selectedClient?.firstName || 'Client'}</strong>
      ...
      <section className="sms-reply-card">
        <div className="table-header">
          <h4>Send SMS reply</h4>
          <span>{selectedClient?.mobile || 'No mobile number'}</span>
        </div>

        <div className="form-grid">
          <div className="full-width">
            <label>SMS template</label>
            <select value={smsTemplateId} onChange={(e) => chooseSmsTemplate(e.target.value)}>
              <option value="">Manual message / choose template</option>
        ...
        <div className="form-actions">
          <button className="secondary" onClick={() => { setSmsBody(''); setSmsTemplateId(''); }}>Clear</button>
          <button className="primary" disabled={sendingSms || !smsBody.trim() || !selectedClient?.mobile} onClick={sendSmsFromThread}>
        ...
        <section className="card client-record-shell premium-panel">
          <div className="client-tabs">
            {[
              ...
              ['documents', 'Documents'],
              ['tasks', 'Tasks'],
              ['sms', 'SMS' + (unreadSmsCount ? ' (' + unreadSmsCount + ')' : '')],
              ['notes', 'Notes'],
              ['activity', 'Activity'],
            ].map(([key, label]) => (
              <button
                ...
                onClick={() => {
                  setClientTab(key as ClientTab);
                  if (key === 'sms' && selectedClientId) {
                    void markClientSmsRead(selectedClientId);
                  }
                }}
              >
                {label}
  ...
  return (
    <>
      ...
      <section className="admin-overview-grid">
        <div className="card premium-panel admin-overview-card">
      ...
      <section className="card premium-panel tab-panel admin-template-panel">
        <div className="table-header admin-section-heading">
          <div>
            <h3>Templates</h3>
            <p>Create and manage SMS and email templates used across the CRM.</p>
          </div>
      ...
      <section className="card premium-panel tab-panel admin-collapsible-panel">
        <button className="admin-collapse-button" onClick={() => setShowCreditorAdmin((prev) => !prev)}>
          <span><strong>Creditor Master List</strong><small>{sortedCreditors.length} creditors saved</small></span>
          <span>{showCreditorAdmin ? 'Hide' : 'Show'}</span>
        </button>

        {showCreditorAdmin && (
          <div className="detail-sections admin-creditor-content">
            <section className="detail-section">
              <div className="table-header">
            ...
            <section className="detail-section">
              <div className="table-header"><h4>Master creditor list</h4><span>{sortedCreditors.length} creditors</span></div>
              <div className="creditor-admin-list">
                {sortedCreditors.map((item) => (
                  <div key={item.id} className="creditor-admin-item"><strong>{item.name}</strong><div className="debt-actions"><button className="secondary small-button" onClick={() => editCreditor(item)}>Edit</button><button className="danger-button small-button" onClick={() => deleteCreditor(item.id)}>Delete</button></div></div>
                ))}
              </div>
            </section>
          </div>
